---
"description": "了解如何使用 Aspose.Words for .NET 将 Word 文档转换为 HTML 时有效解决字体问题。本分步指南提供了清晰的保存选项配置说明，以确保您的字体在导出的 HTML 格式中正确显示。"
"linktitle": "在 HTML 转换中解析字体名称"
"second_title": "Aspose.Words文档处理API"
"title": "在 HTML 转换中解析字体名称"
"url": "/zh/words/net/words-processing-with-htmlsaveoptions/resolve-font-names-in-html-conversion/"
"weight": 10
---

## 介绍

嗨，程序员朋友们！如果您在将 Word 文档保存为 HTML 格式时遇到字体问题，那么您并不孤单。字体问题可能比较棘手，但不用担心；本指南将帮助您使用 Aspose.Words for .NET 解析 Word 文档中的字体名称。让我们逐步深入研究该过程，确保您的字体在 HTML 格式下显示正确。
...
2. 有效许可证：购买许可证 [这里](https://purchase.aspose.com/buy) 或获得临时驾照 [这里](https://purchase。aspose.com/temporary-license/).
...
在编码之前，请确保已将必要的命名空间导入项目。这对于访问 Aspose.Words 功能至关重要。
...
这里， `dataDir` 保存文档目录的路径。替换 `"YOUR_DOCUMENT_DIRECTORY"` 使用系统上的实际路径。

## 第 2 步：加载 Word 文档

接下来，我们需要加载要处理的Word文档。该文档应包含要解析的字体。

```csharp
Document doc = new Document(dataDir + "MissingFont.docx");
```

我们创建一个 `Document` 对象并从我们的 `dataDir`。
