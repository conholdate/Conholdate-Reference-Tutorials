...
using Aspose.Cells;
```
Hiermee hebt u toegang tot alle functionaliteiten die Aspose.Cells biedt.
...
Absoluut! Je kunt zoveel aangepaste XML-onderdelen toevoegen als je nodig hebt, elk met een unieke ID voor eenvoudige toegang.

### Hoe kan ik XML-onderdelen ophalen als ik de ID's niet weet?
Als u de ID's niet kent, kunt u door de `CustomXmlParts` verzameling om beschikbare onderdelen en hun ID's te bekijken, waardoor identificatie gemakkelijker wordt.
